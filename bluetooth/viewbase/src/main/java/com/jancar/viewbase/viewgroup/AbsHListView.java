...
import com.jancar.viewbase.utils.ViewHelperFactory;
import com.jancar.viewbase.utils.v11.MultiChoiceModeListener;
import com.jancar.viewbase.utils.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener,
	...
	/**
	 * Returns the currently checked item. The result is only valid if the choice mode has been set to {@link }.
	 *
	 ...
	 *         list, or <code>null</code> if the choice mode is set to
	 */
	 ...
	 * and the adapter has stable IDs. ({@link ListAdapter#hasStableIds()} == {@code true})
	 *
	 ...
	 * setting the choiceMode to {@link #}, the List allows up to one item to be in a chosen state. By setting the
	 * choiceMode to {@link }, the list allows any number of items to be chosen.
	 *
	 * @param choiceMode
	 */
	@TargetApi(11)
...
//				ss.checkState = mCheckStates.clone();
	...
	/**
	 * Fires an "on scroll state changed" event to the registered , if any.
